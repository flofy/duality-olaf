#!/usr/bin/env python3
"""Inspect the 55 x 130-byte candidate table found in Maouss DS.

The tool deliberately does not write the extracted game data to the
repository. It is intended for local analysis of a user-supplied .nds file.
"""
...
def main() -> None:
    ...
    records = [data[args.offset + i * RECORD_SIZE : args.offset + (i + 1) * RECORD_SIZE]
    ...
    if args.index:
        ...
        print(f"record={args.index} offset=0x{args.offset + (args.index - 1) * RECORD_SIZE:X}")
        print(render(record))
        return

    print(f"offset=0x{args.offset:X} records={LEVEL_COUNT} stride={RECORD_SIZE}")
    for i, record in enumerate(records, 1):
        counts = [record.count(v) for v in range(6)]
        print(f"{i:02d}: " + " ".join(f"{n}={counts[n]}" for n in range(6)))
