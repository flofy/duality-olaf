...
// Import du Burger Menu et Config Screen
...
// Composant PWA Controls déplacé en bas
  ...
  return (
    ...
    >
      ...
      <div className="pwa-actions">
        ...
        {!updateAvailable && !isInstalled && installPrompt && (
          <Button
            icon={<ArrowRight size={16} />}
            label="INSTALLER"
            onClick={async () => {
...
function FullscreenButton() {
  ...
  return (
    <Button
      ...
      onClick={toggle}
      aria-label={
        fullscreen ? "Quitter le plein ecran" : "Passer en plein ecran"
      }
      variant="secondary"
...
// Indicateur d'installation PWA
function PwaInstallIndicator() {
  ...
  return (
    <div className="pwa-installed-indicator" role="status" aria-live="polite">
      <CheckIcon size={16} />
      <span>Installé</span>
    </div>
  );
  ...
  return (
    <main className="app" style={vars()}>
      <div className="shell app-enter">
        <div className="utility-bar top-utility-bar">
          <PwaInstallIndicator />
...
function Help() {
  ...
  const rules: [string, string][] = [
    ...
    ["CHANGER", "ESPACE ou BALLE CARRE choisit la forme active."],
    ["BLOQUER", "La balle et le carre peuvent se servir mutuellement de mur."],
    [
      "PORTES",
...
function Game({ level, worldId }: { level: Level; worldId: number }) {
  ...
  return (
    ...
    >
      ...
      {isDevtoolsEnabled && (
        <aside className="dev-entry" aria-label="Outils de developpement">
          <strong>DEBUG</strong>
          <span>{level.id}</span>
          <span>
            BALLE {state.ball.x},{state.ball.y}
            {level.square
              ? ` - CARRE ${state.square.x},${state.square.y}`
              : " - CARRE absent"}
          </span>
          <span>
