...
import { christmas, doorSwitchTutorials, seasonalEvents, teleporterTutorials } from '@duality/level-format';
...
function difficulty(level: Level) { return validateLevel(level).difficulty; }
...
export function LevelLab({ back }: Props) {
  ...
  return (
    <section className="level-lab">
      <div className="topbar"><button className="action" onClick={back}>← MENU</button><b>LEVEL LAB</b></div>
      ...
      <div className="level-lab-layout">
        <aside className="level-lab-list">
          {groups.map((group) => <section key={group}><b className="level-lab-group">{group}</b>{entries.filter((entry) => entry.group === group).map(({ level }) => { const d = difficulty(level); return <button className={`level-lab-item ${level.id === selected.id ? 'selected' : ''}`} key={level.id} onClick={() => setSelectedId(level.id)}><span>{level.id}</span><small>{d ? `${d.moves} coups · ${d.score}` : 'UNSOLVABLE'}</small></button>; })}</section>)}
        </aside>
        <div className="level-lab-preview">
          <header className="level-lab-header"><div><b>{selected.id}</b><span>{selected.width} × {selected.height}</span></div><div className="level-lab-metrics">{metrics ? `✓ solvable · ${metrics.moves} coups · ${metrics.exploredStates} états · score ${metrics.score}` : '✗ UNSOLVABLE'}</div></header>
...
function LabGame({ level }: { level: Level }) {
  ...
  return <>
    <div className="level-lab-board board" style={{ '--cols': level.width, '--rows': level.height } as CSSProperties}>
    ...
    <div className="level-lab-controls"><div className="hud"><b>{state.activeForm === 'ball' ? '● BOULE' : '■ CARRÉ'}</b><br /><span className="muted">★ {level.stars.length - state.stars.length}/{level.stars.length} · {state.moves} COUPS</span></div><div className="controls"><div className="dpad"><button className="up" onClick={() => move(0, -1)}>▲</button><button onClick={() => move(-1, 0)}>◀</button><button onClick={() => move(0, 1)}>▼</button><button onClick={() => move(1, 0)}>▶</button></div><button className="action switch" onClick={switchForm}>● ⇄ ■<br />CHANGER</button><button className="action" onClick={reset}>↻ RESET</button></div></div>
