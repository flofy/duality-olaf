import { worlds as canonicalWorlds, world3Refined } from '@duality/level-format';
...
export function getWorldStartIndex(worldId: number): number {
  return worlds.filter((world) => world.id < worldId).reduce((total, world) => total + world.levels.length, 0);
}
...
export function levelLabel(index: number): string { return `Niveau ${String(index + 1).padStart(2, '0')}`; }
