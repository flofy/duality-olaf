...
function BoardPreview({ level }: { level: Level }) {
  return (
    <div className="generator-board board" style={{ "--cols": level.width, "--rows": level.height } as CSSProperties}>
...
export function LevelGenerator() {
  ...
  const candidates = useMemo<Candidate[]>(() => {
    ...
    return generated.map((level) => ({ level, validation: validateLevel(level) })).filter((item) => item.validation.result.solvable).sort((a, b) => a.validation.difficulty!.score - b.validation.difficulty!.score).slice(0, 8);
  }, [options, generation]);
  ...
  if (selected)
    return (
      <section className="generator-player">
        <div className="topbar"><button className="action" onClick={() => setSelectedSeed(null)}>← GÉNÉRER</button><b>APERÇU · SEED {selected.level.seed}</b></div>
        <p className="dev-banner">DEV ONLY · collecte les ★ pour terminer le niveau · SEED {selected.level.seed}</p>
        <LabGame level={selected.level} skin="default" themeName="retro" onCompletionChange={() => {}} optimalMoves={selected.validation.difficulty?.moves} onBackToGenerator={() => setSelectedSeed(null)} />
      </section>
    );
  ...
  return (
    <section className="dev-generator">
      <div className="topbar"><button className="action" onClick={() => { window.location.hash = "#/dev/levels"; }}>← LAB</button><b>GÉNÉRATEUR DE NIVEAUX</b></div>
      ...
      <div className="generator-grid">
        {candidates.map(({ level, validation }) => {
          ...
          const scoreClass = difficulty.score <= 120 ? "easy" : difficulty.score <= 200 ? "medium" : "hard";
          return <button className={`generator-card card-${scoreClass}`} key={level.seed} onClick={() => setSelectedSeed(level.seed)}><BoardPreview level={level} /><div className="generator-card-meta"><b>SEED {level.seed}</b><span>✓ {difficulty.moves} coups · score {difficulty.score}</span></div></button>;
        })}
      </div>
