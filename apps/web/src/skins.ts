...
export const skinOrder: readonly SkinPreference[] = [
  "auto",
  "default",
  "halloween",
  "christmas",
...
function isSkinPreference(value: unknown): value is SkinPreference {
  return (
    typeof value === "string" && skinOrder.includes(value as SkinPreference)
  );
}
...
export function getAvailableSkinPreferences(
  date = new Date(),
): readonly SkinPreference[] {
  return skinOrder.filter(
    (preference) => preference === "auto" || isSkinAvailable(preference, date),
  );
}

/** A persisted seasonal override must not survive outside its active window. */
export function normalizeSkinPreference(
  preference: SkinPreference,
  date = new Date(),
): SkinPreference {
  return preference === "auto" || isSkinAvailable(preference, date)
    ? preference
    : "auto";
}
 ...
 */
export function resolveLevelSkin(
  levelId: string,
  preference = getSkinPreference(),
  date = new Date(),
): LevelSkin {
