 ...
 * mounted et se déplace de `from` à `target`, avec une trace de clones sur les
 * cases intermédiaires. Aucun overlay de remplacement ni clignotement ne doit
 * revenir.
 */
test("le sprite principal reste continu et la trace suit tout le trajet", async ({
  ...
  await expect(board.locator(".movement-ghost")).toHaveCount(9);
